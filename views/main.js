...
window.addEventListener("message", async (e) => {
  if (e.data?.type === "EXTRACTION_RESULT") {
    ...
    window._latestGraphics = graphics;
...
function buildTLDRPrompt(text, graphics) {
  const vis = visualsContextForPrompt(graphics);
  return [
    "You are a concise assistant.",
    "Write a short TL;DR summary in clear prose (not bullets).",
    "Keep it factual and include key outcomes and implications.",
    vis ? vis : null,
    "",
    "TEXT:",
    text
...
async function requestSummarizeText(text, source) {
  ...
  if (!text.trim()) {
    document.getElementById("out").textContent = (source === "selection")
      ? "No text selected to summarize."
      : "No page content to summarize.";
    return;
  }

  document.getElementById("out").textContent = `Summarizing ${source}...`;

  const { summaryType = "key-points" } = await chrome.storage.local.get("summaryType");

  if (summaryType === "key-points") {
    // Summarizer API
    const summaryLength = document.querySelector('input[name="len"]:checked').value;
    window.parent.postMessage({
      type: "REQUEST_SUMMARIZATION",
      summaryLength,
      text
    }, "*");
  } else {
    // Prompt API (LanguageModel) for TL;DR
    const prompt = buildTLDRPrompt(text, window._latestGraphics || []);
    window.parent.postMessage({
      type: "REQUEST_LM_SUMMARY",
      text: prompt
    }, "*");
  }
}
